...
class ExportNdpContactsWizard(models.TransientModel):
    ...
    def generate_csv(self):
        ...
        for partner in self.partner_ids:
            ...
            # Split apellido / nombre vendedor – versión segura y limpia
            apellido_vend = ""
            nombre_vend = ""
            if partner.user_id and partner.user_id.name:
                name = partner.user_id.name.strip()
                if ", " in name:
                    parts = name.split(", ", 1)
                    apellido_vend = parts[0].strip()
                    nombre_vend = parts[1].strip() if len(parts) > 1 else ""
                else:
                    parts = name.split()
                    if len(parts) > 1:
                        apellido_vend = " ".join(parts[:-1]).strip()
                        nombre_vend = parts[-1].strip()
                    else:
                        apellido_vend = name

            # Tags para zona y categoría (ordenados por ID, como en los ejemplos)
            tags_sorted = partner.category_id.sorted("id")
            zona = str(tags_sorted[0].id if tags_sorted else 1)
            categoria = str(
                tags_sorted[1].id
                if len(tags_sorted) > 1
                else (tags_sorted[0].id if tags_sorted else 1)
            )

            # Formateo de montos sin .00 si es entero (ej: 500000 en vez de 500000.00)
            def fmt_amount(amount):
                if not amount or amount == 0:
                    return "0"
                s = f"{abs(amount):.2f}"  # abs por si saldo negativo, pero en ejemplos siempre positivo
